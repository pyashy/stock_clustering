...
class TickerDataset(Dataset):
    ...
    def __init__(self, data_file: str, time_period: int):
    ...
    def preprocessing(self) -> None:
        self.data = self.data.dropna(axis=1).pct_change().fillna(0).T.values
...
class TickerDataModule(LightningDataModule):
    ...
    def __init__(self, data_path: str, batch_size: int = 16, time_period: int = 30):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        train_val, self.test = train_test_split(TickerDataset(self.data_path, self.time_period), train_size=0.8)
